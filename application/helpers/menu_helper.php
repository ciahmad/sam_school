...
if (!function_exists('active_link')) {
    ...
    function menu_heading($menu_heading) { 
        $CI = get_instance();
        $session_menu_heading = $CI->session->userdata('menu_heading');
        if ($session_menu_heading == $menu_heading) {
    ...
    function sub_heading($sub_heading) { 
        $CI = get_instance();
        $session_menu_heading = $CI->session->userdata('sub_heading');
        if ($session_menu_heading == $sub_heading) {
            return 'display: block;';
        }
        return "";
    }
    function set_1stLevel($sub_menu_name) {
        $CI = get_instance();
        $session_sub_menu_name = $CI->session->userdata('sub_menu');
        if ($session_sub_menu_name == $sub_menu_name) {
            return 'background: #070707;border-left: 5px solid #DC700E;color: #faa21c;';
        }
        return "";
    }

    function set_2ndLevel($sub_submenu_name) {
        $CI = get_instance();
        $session_sub_submenu_name = $CI->session->userdata('subsub_menu');
        if ($session_sub_submenu_name == $sub_submenu_name) {
            return 'background: #070707;border-left: 5px solid #DC700E;color: #faa21c;';
        }
        return "";
    }
