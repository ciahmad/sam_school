        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($no_of_partners > 0) {
                ...
                for ($i=1; $i <= $no_of_partners; $i++) { 
                    ...
                    $partnersData[] = array('name'=>'Partner Name-'.$i.'', 'value'=>$this->input->post('partner_name'.$i));
                    $partnersData[] = array('name'=>'% of Shares-'.$i.'', 'value'=>$this->input->post('part_shares'.$i).'%');
                }
                if($percent_value > 100){
                    $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('percent_message'));
                    echo json_encode($array);
                }else{
            ...
            if ($no_of_directors > 0) {
                for ($i=1; $i <= $no_of_directors; $i++) { 
                    $directsData[] = array('name'=>'Directors Name-'.$i.'', 'value'=>$this->input->post('director_name'.$i));
                    $directsData[] = array('name'=>'% of Shares-'.$i.'', 'value'=>$this->input->post('dir_shares'.$i).'%');
                }
