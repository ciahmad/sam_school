...
class Schsettings extends Admin_Controller {
    ...
    public function index() {
        ...
        $businessTerms = $this->setting_model->getBusinessTerms();
        if($businessTerms){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($no_of_partners > 0 && $no_of_partners!='') {
                ...
                for ($i=1; $i <= $no_of_partners; $i++) { 
                    ...
                    $partnersData[] = array('name'=>'Partner Name-'.$i.'', 'value'=>$this->input->post('partner_name'.$i), 'textfield_name'=>'partner_name'.$i.'');
                    $partnersData[] = array('name'=>'% of Shares-'.$i.'', 'value'=>$this->input->post('part_shares'.$i), 'textfield_name'=>'part_shares'.$i.'');
                }
            ...
            if ($no_of_directors > 0 && $no_of_directors!='') {
                ...
                for ($i=1; $i <= $no_of_directors; $i++) { 
                    ...
                    $directsData[] = array('name'=>'Director Name-'.$i.'', 'value'=>$this->input->post('director_name'.$i), 'textfield_name'=>'director_name'.$i.'');
                    $directsData[] = array('name'=>'% of Shares-'.$i.'', 'value'=>$this->input->post('dir_shares'.$i), 'textfield_name'=>'dir_shares'.$i.'');
                }
                if($_percent > 100){
                    $errorMsg   = array('errorMsg'=>$this->lang->line('percent_message'));
                    $array      = array('status' => 'fail', 'error' => $errorMsg);
