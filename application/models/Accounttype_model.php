...
class Accounttype_model extends MY_Model {
    ...
    public function getCategories($parent_id = 0, $user_id) {

        $this->db->select('id, name, parent_account_type_id');
        $this->db->from('account_types');
        $where = "parent_account_type_id = ".$parent_id." and user_id =".$user_id." ";
        $this->db->where($where);
        $query = $this->db->get();
        //echo $this->db->last_query(); die();
        if ($query->num_rows() > 0) {
